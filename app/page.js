...
function HomeContent() {
  ...
  const loadSchedule = async (groupId = null) => {
        ...
        <html>
          ...
          <body>
            ...
            <table border="1" cellpadding="8">
              <thead><tr><th>День</th><th>Время</th>${activeTab !== 'my-lessons' ? '<th>Группа</th>' : ''}<th>Предмет</th><th>Преподаватель</th><th>Аудитория</th><th>Заметки</th></tr></thead>
              <tbody>
                ${exportData.map(lesson => `
                  <tr>
                    <td>${DAYS[lesson.day_of_week - 1]}</td>
                    <td>${PAIRS[lesson.pair_number - 1].time}</td>
                    ${activeTab !== 'my-lessons' ? `<td>${lesson.group_name}</td>` : ''}
  ...
  const handleAddLesson = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        await loadSchedule(selectedGroupFilter);
          ...
          <div className="landing-content">
            <div className="landing-hero">
              ...
              <p className="hero-description">
                Современная платформа для просмотра расписания
              </p>
          ...
          <div className="modal-container" onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={editingLesson ? handleUpdateLesson : handleAddLesson} className="modal-form">
              ...
              <button type="submit" className="submit-btn">
                <i className="fas fa-save"></i> {editingLesson ? ' Сохранить' : ' Добавить'}
              </button>
            </form>
          </div>
        </div>,
      ...
      {showGroupModal && createPortal(
        <div className="modal" onClick={() => setShowGroupModal(false)}>
          <div className="modal-container" onClick={e => e.stopPropagation()}>
            ...
            <div className="modal-form">
              <div className="form-group"><label>Название</label><input value={newGroup} onChange={e => setNewGroup(e.target.value)} autoFocus /></div>
              <button className="submit-btn" onClick={() => addDirectory('groups', newGroup, setShowGroupModal, setNewGroup)}>Добавить</button>
            </div>
          </div>
        </div>,
        document.body
      ...
      {showTeacherModal && createPortal(
        <div className="modal" onClick={() => setShowTeacherModal(false)}>
          <div className="modal-container" onClick={e => e.stopPropagation()}>
            ...
            <div className="modal-form">
              <div className="form-group"><label>ФИО</label><input value={newTeacher} onChange={e => setNewTeacher(e.target.value)} autoFocus /></div>
              <button className="submit-btn" onClick={() => addDirectory('teachers', newTeacher, setShowTeacherModal, setNewTeacher)}>Добавить</button>
            </div>
          </div>
        </div>,
        document.body
      ...
      {showSubjectModal && createPortal(
        <div className="modal" onClick={() => setShowSubjectModal(false)}>
          <div className="modal-container" onClick={e => e.stopPropagation()}>
            ...
            <div className="modal-form">
              <div className="form-group"><label>Название</label><input value={newSubject} onChange={e => setNewSubject(e.target.value)} autoFocus /></div>
              <button className="submit-btn" onClick={() => addDirectory('subjects', newSubject, setShowSubjectModal, setNewSubject)}>Добавить</button>
            </div>
          </div>
        </div>,
        document.body
        ...
        <div className="modal" onClick={() => setShowClassroomModal(false)}>
          <div className="modal-container" onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={handleAddClassroom} className="modal-form">
              <div className="form-group"><label>Номер</label><input value={newClassroom} onChange={e => setNewClassroom(e.target.value)} autoFocus /></div>
