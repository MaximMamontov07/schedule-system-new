...
// Получить все исключения для группы
export async function GET(request) {
  try {
    ...
    if (teacherId) {
      query += ` AND (e.teacher_id = $${idx++} OR e.replacement_teacher_id = $${idx++})`;
      params.push(parseInt(teacherId), parseInt(teacherId));
    }
    if (startDate) {
      query += ` AND e.exception_date >= $${idx++}`;
...
export async function POST(request) {
  try {
    ...
    const { 
      ...
      pair_number, exception_date, exception_type,
      replacement_teacher_id, replacement_subject_id, replacement_classroom_id,
      notes
    } = body;

    // Проверяем, нет ли уже исключения на эту дату/пару
    ...
    if (existing.rows.length > 0) {
      ...
      await db.query(`
        ...
        SET exception_type = $1, teacher_id = $2, subject_id = $3, 
            classroom_id = $4, replacement_teacher_id = $5, 
            replacement_subject_id = $6, replacement_classroom_id = $7,
            notes = $8
        WHERE id = $9
      `, [
        exception_type,
...
export async function DELETE(request) {
  try {
    ...
    const db = await getDb();
    await db.query('DELETE FROM schedule_exceptions WHERE id = $1', [id]);
